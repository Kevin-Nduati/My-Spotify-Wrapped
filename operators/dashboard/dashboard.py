...
newly_discovered_tracks = pd.read_sql_query(
    "SELECT * FROM newly_discovered_tracks", conn
)
newly_discovered_artists = pd.read_sql_query(
    "SELECT DISTINCT(artist_name) FROM newly_discovered_artists", conn
)
...
ax4.set_yticklabels(
    ax4.get_yticklabels(), fontsize=16, fontweight="bold", fontfamily="serif"
)


# axis 5 values
table = ax5.table(
    cellText=newly_discovered_tracks.head(10).values,
    colLabels=["Track Name"],
    loc="center",
)
...
# axis 6 values
table = ax6.table(
    cellText=newly_discovered_artists.head(10).values,
    colLabels=["Artist Name"],
    loc="center",
)
...
ax7.set_title(
    "Listening Habits by Day and Hour",
    fontweight="bold",
    fontsize=35,
    color=color1,
    fontfamily="serif",
...
ax7.set_yticklabels(
    ax7.get_yticklabels(), fontsize=12, fontweight="bold", fontfamily="serif"
)
ax7.set_xticklabels(
    ax7.get_xticklabels(), fontsize=16, fontweight="bold", fontfamily="serif"
)
